<x-layouts.public>
    <x-breadcrumb :breadcrumbs="['Home', 'Daftar Anggota']" />
    <section>
        <div class="mx-auto max-w-8xl px-5 pt-2 pb-8 md:px-10 md:pt-4 md:pb-10">
            <h2 class="text-center text-3xl font-bold text-gray-700">Daftar Anggota Teregistrasi per-BUJK</h2>
            <div class="flex flex-col p-8 rounded-xl space-y-10">
                <form id="filter-form" class="mb-4 flex flex-col md:flex-row gap-3 md:items-center">
                    ...
                    <select name="sort_by" class="px-3 py-2 border rounded-lg dark:bg-gray-700 dark:text-white">
                        <option value="nama_bujk" {{ request('sort_by') == 'nama_bujk' ? 'selected' : '' }}>Sort by Nama</option>
                        <option value="nama" {{ request('sort_by') == 'nama' ? 'selected' : '' }}>Sort by Provinsi</option>
                        <option value="kualifikasi" {{ request('sort_by') == 'kualifikasi' ? 'selected' : '' }}>Sort by Kualifikasi</option>
                    </select>
                ...
                <div id="anggota-tabel">
                    @include('public.anggota.partials.table', [
                        'data' => $data,
    ...
    <script>
        ...
        const tableWrapper = document.getElementById('anggota-tabel');
    
        function fetchData() {
            const formData = new FormData(form);
            const params = new URLSearchParams(formData).toString();
    
            fetch("{{ route('anggota.bujk.index') }}?" + params, {
