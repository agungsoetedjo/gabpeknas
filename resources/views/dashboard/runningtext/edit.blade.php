<x-layouts.admin>
    <div class="p-4 sm:ml-64">
        ...
        <div class="mt-6 p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <form action="{{ route('dashboard.runningtext.update', $runningtext->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label for="teks" class="block text-sm font-medium text-gray-700">Teks Running</label>
                    <textarea id="teks" name="teks" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200 focus:border-blue-400">{{ old('teks', $runningtext->teks) }}</textarea>
                    @error('teks')
