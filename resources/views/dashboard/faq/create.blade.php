<x-layouts.admin>
    <div class="p-4 sm:ml-64">
        ...
        <div class="mt-6 p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
            <form action="{{ route('dashboard.faq.store') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="answer" name="answer" rows="5"
                        class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring focus:ring-blue-200 focus:border-blue-400">{{ old('answer') }}</textarea>
                    @error('answer')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
